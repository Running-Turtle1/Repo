...
{
	u8 time_temp=0;
	//// 满足 60us<延时<240us, 等待DQ为低电平
	//// 或者说是检测  DS18 是否占用总线, 发送低电平应答信号
...
* 输    入         : 无
* 输    出         : 温度数据
DS18B20 的典型温度读取过程为：复位→发 SKIP ROM 命令（0XCC）→发开始转 换命令（0X44）→延时→复位
→发送 SKIP ROM 命令（0XCC）→发读存储器命令 （0XBE）→连续读出两个字节数据(即温度)→结束。
*******************************************************************************/
